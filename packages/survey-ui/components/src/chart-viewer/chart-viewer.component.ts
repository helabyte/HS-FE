  ...
  implements OnChanges, AfterViewInit, OnDestroy
{
  ...
  private renderChart(): void {
    ...
    const data = this.chartData.options.map((option: any) => ({
      label: option.label,
    ...
    const validData = data.filter(
      (d: any) => typeof d.votes === 'number' && d.votes >= 0
    );
  ...
  private renderPieChart(
    ...
    const pie = d3.pie().value((d: any) => d.votes); // Use 'votes' for the pie slice size
    const arc = d3
      .arc()
    ...
    arcs
      .append('path')
      .attr('d', arc)
      .attr('fill', (d: any, i: number) => d3.schemeCategory10[i % 10]) // Use a color scheme
      .attr('class', 'cursor-pointer') //for show on hover data of each part of chart
      .append('title') //tooltip, appears on hover
      .text((d: any) => `${d.data.label}: ${d.data.votes} votes`); // Show votes in tooltip
  ...
  private renderBarChart(
    ...
    this.svg
      ...
      .attr('y', (d: any) => y(d.votes)) // Use 'votes' for bar height
      .attr(
        'height',
        (d: any) => height - margin.top - margin.bottom - y(d.votes)
      )
      .attr('fill', (d: any, i: number) => d3.schemeCategory10[i % 10]) // Consistent colors
      .append('title') // Add a tooltip
      .text((d: any) => `${d.label}: ${d.votes} votes`); // Show votes in tooltip
    ...
    this.svg
      ...
      .attr('y', (d: any) => y(d.votes) - 5) // Position above the bar
  ...
  private renderLineChart(
    ...
    const line = d3
      .line()
      .x((d: any) => x(d.label)! + x.bandwidth() / 2) // Center the line points
      .y((d: any) => y(d.votes)) // Use 'votes' for y-coordinate
